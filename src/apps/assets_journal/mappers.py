...
def map_stationary_asset_domain_to_response(domain: StationaryAssetDomain) -> StationaryAssetResponseSchema:
    """Маппинг доменной модели в схему ответа"""
    return StationaryAssetResponseSchema.model_validate(domain, from_attributes=True)
...
def map_create_schema_to_domain(create_schema: CreateStationaryAssetSchema) -> StationaryAssetDomain:
    ...
    return StationaryAssetDomain(
        ...
        actual_datetime=create_schema.actual_datetime,
...
def map_bg_response_to_domain(bg_data: dict) -> StationaryAssetDomain:
    """Маппинг ответа BG в доменную модель (адаптация под новую структуру)"""
    ...
    def parse_time(date_str):
        if not date_str:
            return time(9, 0)  # дефолтное время
        try:
    ...
    return StationaryAssetDomain(
        ...
        receive_time=parse_time(bg_data.get("regDate")),
        actual_datetime=parse_datetime(bg_data.get("hospitalDate")),
        received_from=bg_data.get("orgHealthCareRequest", {}).get("name", ""),
        is_repeat=False,  # дефолтное значение
        stay_period_start=parse_datetime(bg_data.get("hospitalDate")),
        stay_period_end=parse_datetime(bg_data.get("outDate")),
        stay_outcome="Лечение",  # дефолтное значение, можно адаптировать
        diagnosis=bg_data.get("sick", {}).get("name", ""),
        area="Общий",  # дефолтное значение, можно адаптировать
